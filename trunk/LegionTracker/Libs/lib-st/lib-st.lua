...
do 
		...
		if sortby then 
			table.sort(self.sorttable, function(a,b)
				...
				if column.comparesort then 
					return column.comparesort(a, b, sortby);
				else
	...
	local CompareSort = function (self, a, b, col)
		local cella = self.data[a].cols[col];
		local cellb = self.data[b].cols[col];
		...
		if a1 == b1 and column.sortnext and (not(self.cols[column.sortnext].sort)) then 
			...
			if nextcol.comparesort then 
				return nextcol.comparesort(a, b, column.sortnext);
			else
				return self:CompareSort(a, b, column.sortnext);
			end
		else
